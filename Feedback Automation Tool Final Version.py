...
import os, re
import time
SHARED_ROOT = r"C:\Users\MohammedShakeelMulla\OneDrive - Erulearning Solutions Pvt Ltd\Feedback Automation Tool Reports"
...
""", unsafe_allow_html=True)
    ...
    shared_root_input = st.text_input(
    "📤 Shared Reports Folder (OneDrive)",
...
if uploaded and generate_btn:
    ...
    for i, ((course, pl, date_str, lob), grp) in enumerate(groups, start=1):
        ...
        wb.save(file_path)
                # ---------------------------------------------------------
        # COPY REPORT TO SHARED ONEDRIVE FOLDER (Phase 2 - Option A)
        # ---------------------------------------------------------
        try:
            ...
            shared_lob_dir.mkdir(parents=True, exist_ok=True)

            shared_copy_path = shared_lob_dir / (base_name + ".xlsx")

            with open(file_path, "rb") as src, open(shared_copy_path, "wb") as dst:
                dst.write(src.read())
