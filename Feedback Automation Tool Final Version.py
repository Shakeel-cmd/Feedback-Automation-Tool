...
import os, re
import time# app_v1_1_4.py — Feedback Automation Tool (Professional Live Dashboard Release)
# Developed by EMERITUS — Feedback Automation Tool
...
def upload_to_sharepoint(local_file_path, session_date, lob):
    """
    Uploads a locally saved feedback report to SharePoint
    ...
    # Parse session date to folder names
    try:
        d = pd.to_datetime(session_date)
        month_year = d.strftime("%B %Y")            # January 2026
        month_day_year = d.strftime("%B %-d, %Y")   # January 2, 2026
    except Exception:
        month_year = "Unknown Period"
        month_day_year = "Unknown Date"

    file_name = Path(local_file_path).name

    # Build SharePoint upload path
    sharepoint_path = (
        f"{month_year}/"
        f"{month_day_year}/"
        f"{lob}/"
...
if uploaded and generate_btn:
    ...
    for i, ((course, pl, date_str, lob), grp) in enumerate(groups, start=1):
        ...
        lob=lob
            ...
            st.warning(f"⚠️ SharePoint upload failed: {e}")
    ...
    if template_path.exists():
        with open(template_path, "rb") as tfile:
            st.download_button(
                ...
                file_name="Feedback_Automation Template.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="template_download_btn_sidebar"
            )
    else:
        st.warning("Template file not found — please add 'Automation Template.xlsx' to the app folder.")
